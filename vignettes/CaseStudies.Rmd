...
As observed in the first Monte Carlo experiment, the success rates obtained when the selection process of this second experiment considered all 16 candidate models were only slightly lower than those found when it considered models 1 to 4. For both selection processes, the success rates for all simulation rounds remained close to 90% for k values greater than 4. As in the first experiment, this suggests that the AICc (k > 4) avoided overfitting. The results of the two Monte Carlo experiments led us to select 4.4 as the optimal penalty factor for calculating the AICc within the SPIChanges() function.
...
download_and_process <- function(year, lonlat) {
  ...
  for (i in seq_len(n)) {
    ...
    pdata[, i] <- prate[which.min((lons - longitude)^2),
...
colnames(Map) <- c("lon", "lat", "SummerModerate", "SummerSevere", "SummerExtreme","SummerModel",
...
library(archive)
# Loading data
url <- "https://raw.githubusercontent.com/gabrielblain/Grid_Brazil/refs/heads/main/Map.csv"
Map <- read.csv(url,sep=",", header = TRUE)
...
#Tidding the data
df <- Map %>%
  select(lat, lon, SpringSevere, OutomnSevere, WinterSevere, SummerSevere) %>%
  pivot_longer(cols = c(SpringSevere, OutomnSevere, WinterSevere, SummerSevere),
...
dados.sf$Station<- factor(dados.sf$Station, levels= c('Spring - Severe','Summer - Severe','Autumm - Severe','Winter - Severe'))
...
map <- ggplot() +
  ...
  scale_colour_gradient2(low = "#6B238E", mid = "white", high = "red", 
...
map <- map + 
  geom_text(data = limitere_centroids, aes(x = lon, y = lat, label = sigla), 
            color = "black", size = 3, 
            fontface = "bold", 
