...
SPI values. In other words, the `SPIChanges` package leverages the
...
this background, the key function of this package is `SPIChanges()`,
...
```
    ...
    ## 1 1991     1         4      406.3 1.48          0.93             0.86
    ## 2 1991     2         1      517.7 2.42          0.99             0.99
    ## 3 1991     2         2      530.5 2.44          0.99             0.83
    ## 4 1991     2         3      455.5 2.01          0.98             0.90
    ## 5 1991     2         4      242.5 0.75          0.77             0.55
    ## 6 1991     3         1      202.5 0.49          0.69             0.69
    ##   ChangeFreq
...
```
    ...
    ## [4,]     1         4   -12.83  -6.43   -2.29
    ## [5,]     2         1     0.00   0.00    0.00
    ## [6,]     2         2    17.19   7.56    2.22

# Details

The NSPI uses the same calculation algorithm as the SPI. However, the
stationary gamma distribution is replaced by non-stationary versions,
which allow its parameters to vary as a function of a covariate. In this
package, the gamma distributions are fitted to rainfall data using a
Generalized Additive Model, with time as covariate. This fitting process
is based on the maximum likelihood method (McCullagh and Nelder, 1989)
and considers the following gamma-based models (candidate models).

ToDo: Describe the non-stationary models!
